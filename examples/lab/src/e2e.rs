use bevy::prelude::*;
use saddle_saddle_bevy_e2e::{E2EPlugin, E2ESet, init_scenario};

use crate::scenarios::{list_scenarios, scenario_by_name};

pub struct SpritesheetLabE2EPlugin;

impl Plugin for SpritesheetLabE2EPlugin {
    fn build(&self, app: &mut App) {
        app.add_plugins(E2EPlugin);
        app.configure_sets(
            Update,
            E2ESet.before(saddle_animation_spritesheet::SpritesheetSystems::ResolveRequests),
        );
        ...
        if let Some(name) = scenario_name {
            if let Some(mut scenario) = scenario_by_name(&name) {
                ...
                error!(
                    "[saddle_animation_spritesheet_lab:e2e] Unknown scenario '{name}'. Available: {:?}",
                    list_scenarios()
                );
            }
        }
